...
<script lang="ts">
	...
	interface Props {
		...
		text?: string;
		/**
		 * Display type of the text element
		 * @default 'inline'
		 */
		...
		/**
		 * Text alignment (only applies when display is 'block')
		 * @default 'left'
		 */
		...
		loading?: boolean;
		/**
		 * Whether the text is disabled (reduced opacity)
		 * @default false
		 */
		disabled?: boolean;
		/**
		...
		ariaLabel?: string;
		/**
		 * Hide text from screen readers (decorative text)
		 * @default false
		 */
		ariaHidden?: boolean;
		/**
	...
	// Text classes using static Tailwind/DaisyUI classes
	let textClasses = $derived(
		[
			...
			size === 'xl' && 'text-xl',
			size === '2xl' && 'text-2xl',
			// Alignment (only applies to block display)
			display === 'block' && align === 'left' && 'text-left',
			display === 'block' && align === 'center' && 'text-center',
			display === 'block' && align === 'right' && 'text-right',
			display === 'block' && align === 'justify' && 'text-justify',
	...
	// Determine if text is decorative
